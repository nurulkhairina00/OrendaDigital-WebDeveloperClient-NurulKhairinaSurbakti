  ...
  const handleSubmit = () => {
    axios({
      ...
      .then(() => {
        Swal.fire({
          title: "Success",
          text: "Successfully updates orders data",
          icon: "success",
          timer: 1500,
...
export async function getServerSideProps(context) {
  ...
  const order = await getDataOrderById(context.query.id);
  const listCustomer = await getDataCustomer();
  const listProduct = await getDataProduct();
  console.log("order EDIT", order);
  return {
    props: { session, order, listCustomer, listProduct },
  };
}

const getDataOrderById = async (id) => {
  return await axios
    .get(`${process.env.API_PATH}:${process.env.API_PORT}/api/orders/${id}`)
    .then((res) => {
      return res.data;
    })
    .catch((error) => {
